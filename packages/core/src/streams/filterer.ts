  ...
  public async filter(
    ...
    const getAnimeEntryBareEpisodeAlias = (
      ...
      const matchedAlias = aliases.find((alias) => {
        ...
        return new RegExp(
          `(?:^|\\s)${escapedAlias}\\s+(?:e\\s*)?0*${normaliseBareEpisodeText(String(animeEpisode))}(?:v\\d+)?(?:\\s|$)`,
          'u'
    ...
    if (
      ...
      logger.debug('Title ambiguity episode lookups', {
        id,
        candidates: episodeConflicts.map((conflict) => ({
          title: conflict.title,
          tvdbId: conflict.tvdbId ?? null,
          tmdbId: conflict.tmdbId ?? null,
        })),
        bounds: [...conflictNumberingBounds].map(([tvdbId, bound]) => ({
          tvdbId,
          bound: bound.episode,
          seasonBound: bound.season,
        })),
      });
    }

    if (titleOptions?.enabled) {
      logger.debug('Title ambiguity diagnostics v1', {
        id,
        type,
        isAnime,
        ambiguousResults: titleOptions.ambiguousResults ?? null,
        conflictsEnabled: appConfig.metadata.titleConflicts.enabled,
        requestTypes: titleOptions.requestTypes ?? [],
        titleRequestType,
        ...
        conflicts: [...titleConflicts].map(([title, entries]) => ({
          title,
          entries: entries.map((conflict) => ({
            title: conflict.title,
            year: conflict.year ?? null,
            country: conflict.country ?? null,
            tvdbId: conflict.tvdbId ?? null,
            tmdbId: conflict.tmdbId ?? null,
          })),
        })),
    ...
    const performTitleMatch = (stream: ParsedStream) => {
      ...
      const animeEntryBareEpisodeAlias = getAnimeEntryBareEpisodeAlias(stream);
      ...
      if (!result.matched) {
        logger.debug('Title matching rejected: title mismatch', {
          id,
          filename: stream.filename,
      ...
      if (
        ...
        logger.debug('Title matching rejected: country mismatch', {
          id,
          filename: stream.filename,
          streamCountry,
          requestedCountry,
      ...
      if (
        ...
        logger.debug('Title ambiguity identity verdict', {
          id,
          filename: stream.filename,
          normalisedStreamTitle,
          seasons: stream.parsedFile?.seasons ?? [],
          episodes: stream.parsedFile?.episodes?.slice(0, 10) ?? [],
          episodeCount: stream.parsedFile?.episodes?.length ?? 0,
          date: stream.parsedFile?.date ?? null,
          year: evidence.year ?? null,
          country: evidence.country ?? null,
          matchingEpisode: evidence.episode ?? null,
          matchingSeason: evidence.season ?? null,
          episodeTitleMatches: evidence.episodeTitleMatches,
          confirmed,
          competitors: conflicts.map((conflict) => ({
            title: conflict.title,
            year: conflict.year ?? null,
            country: conflict.country ?? null,
            tvdbId: conflict.tvdbId ?? null,
            tmdbId: conflict.tmdbId ?? null,
            seasonBound:
              conflict.tvdbId === undefined
                ? null
                : (conflictNumberingBounds.get(conflict.tvdbId)?.season ??
                  null),
            episodeBound:
              conflict.tvdbId === undefined
                ? null
                : (conflictNumberingBounds.get(conflict.tvdbId)?.episode ??
                  null),
          })),
        });
