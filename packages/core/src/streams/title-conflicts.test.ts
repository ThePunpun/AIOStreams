...
describe('StreamFilterer ambiguity settings', () => {
  it('checks recovered bare aliases without losing custom episode recovery', async (t) => {
    ...
    t.mock.method(SkyhookMetadata.prototype, 'search', async () => [
      { title: 'Shared Name', year: 2025, country: 'CN', tvdbId: 999999 },
    ]);
    t.mock.method(TMDBMetadata.prototype, 'searchSeries', async () => []);
    t.mock.method(TVDBMetadata.prototype, 'searchSeries', async () => [
      { name: 'Shared Name', year: 2025, country: 'CN', tvdbId: 999999 },
    ]);
    const metadata = {
      title: 'Distinctive Anime',
      titles: [{ title: 'Distinctive Anime' }, { title: 'Shared Name' }],
      year: 2005,
      country: 'JP',
    };
    const context = {
      ...
      id: 'kitsu:1:1',
      isAnime: true,
      animeEntry: { title: 'Distinctive Anime', synonyms: ['Shared Name'] },
      animeEpisode: 1,
    ...
    for (const ambiguousResults of ['keep', 'discard'] as const) {
      ...
      const streams = [
        'Distinctive Anime-01',
        'Shared Name-01',
        'Distinctive Anime-08',
      ].map((filename, i) => ({
        id: String(i),
        type: 'usenet',
        filename,
      ...
      assert.deepEqual(
        ...
        ambiguousResults === 'keep' ? ['0', '1'] : ['0']
      );
    }
  });
  it('prefers the full bare-episode alias over its ambiguous suffix', async (t) => {
    t.mock.method(SettingsRepository, 'getAll', async () => []);
    t.mock.method(SettingsRepository, 'getVersion', async () => 0);
    await settingsStore.initialise();
    t.mock.method(RegexAccess, 'isRegexAllowed', async () => false);
    ...
    const metadata = {
      title: 'Distinctive Shared Name',
      titles: [{ title: 'Distinctive Shared Name' }, { title: 'Shared Name' }],
      year: 2005,
      country: 'JP',
    };
    const context = {
      ...
      animeEntry: {
        ...
        synonyms: ['Shared Name', 'Distinctive Shared Name'],
      },
      animeEpisode: 1,
      parsedId: { season: '1', episode: '1' },
      getMetadata: async () => metadata,
    ...
    for (const ambiguousResults of ['keep', 'discard'] as const) {
      ...
      const streams = [
        'Distinctive Shared Name-01',
        'Shared Name-01',
        'Distinctive Shared Name-08',
      ].map((filename, i) => ({
        id: String(i),
        type: 'usenet',
        filename,
